...
Skeleton provides default styles for inputs and buttons. We need to remove these defaults so we can apply our own custom styles. We'll need to locate the input styles in base.css, and remove the default properties and values:
...
Now we can add our own. We'll round off the corners of our submit buttons first. The properties with "background-clip" will ensure the color stops at the border, preventing it from bleeding past. We'll change the background color to blue and the font color to white:
...
We adjusted the padding within our buttons to sufficiently space them. Buttons have a default styles generated by the browser. One of these styles is a nasty-looking border. We effectively removed the border by giving it a value of 0.

Next, we'll update how our button looks when a user hovers over it. By default, Skeleton provides a simple gradient button with a gradient color change on hover. Since we're implementing a flat design, we'll remove all the unnecessary styles, and add a varied shade of blue as our button color:
...
To match our button style, we'll round the corners of our text input. We'll also add our new font type: Lato.
...
Skeleton provides a default width for input fields. We removed the default width and applied a custom percentage width. We opted for 90% to fill most of column it's residing in. We also updated the padding so that it will be similar in height to that of the button.
